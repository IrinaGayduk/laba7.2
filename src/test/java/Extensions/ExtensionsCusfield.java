...
public class ExtensionsCusfield extends DefaultFieldDecorator {
    public ExtensionsCusfield(SearchContext searchContext) {
    ...
    protected Element createElement(ClassLoader loader, ElementLocator locator, Class<Element> clazz) {
        ...
        return ExtensionsWrapperFactory.createInstance(clazz, proxy);
    }

    @SuppressWarnings("unchecked")
    protected List<Element> createList(ClassLoader loader, ElementLocator locator, Class<Element> clazz) {
        InvocationHandler handler = new ExtensionsLocationCusEl(locator, clazz);
